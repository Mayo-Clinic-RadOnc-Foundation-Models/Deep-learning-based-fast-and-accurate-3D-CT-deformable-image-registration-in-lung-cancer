...
def load_volfile(
    ...
    data_dir = '/rodata/azradonc_dev/m263425/DIR/lung_data/data/'
    pid = filename.split(':')[0]
    filename = filename.split(':')[1]
    filename = filename.split(' ')
   # pdb.set_trace()
    #pid = os.path.basename(filename)
    if len(filename)>2:
        iid = np.random.randint(len(filename),size = 2)
    else:
        iid = np.array([0,1],dtype = np.int8)
    dcm50dir = data_dir+ filename[iid[0]]
    dcm00dir = data_dir +filename[iid[1]]
   # print(filename[iid[0]])
   # print(filename[iid[1]])
 
    dicomData50 = getDicomData(dcm50dir,pid)
    dicomData00 = getDicomData(dcm00dir,pid)
    
    vol50, ctGrid50, ctOrien50 = readWholeCTFunction(dicomData50['CT'])
    vol00, ctGrid00, ctOrien00 = readWholeCTFunction(dicomData00['CT'])
    vol50 = (vol50-vol50.min())/(vol50.max()-vol50.min())
    vol00 = (vol00-vol00.min())/(vol00.max()-vol00.min())

    default_z=160#192
    if vol50.shape[0]!=default_z or vol00.shape[0]!=default_z:
        diff = vol50.shape[0] - default_z
        if diff >0:
            vol50 = vol50[int(diff/2):int(diff/2)+default_z,:,:]
        elif diff <0:
            a_zeros = np.zeros((int(-diff/2),vol50.shape[1],vol50.shape[2]))
            p_zeros = np.zeros((int(diff/2)-diff,vol50.shape[1],vol50.shape[2]))
            vol50 = np.concatenate((a_zeros,vol50,p_zeros),axis = 0)
        diff = vol00.shape[0]-default_z
        if diff >0:
            vol00 = vol00[int(diff/2):int(diff/2)+default_z,:,:]
        elif diff <0:
            a_zeros = np.zeros((int(-diff/2),vol00.shape[1],vol00.shape[2]))
            p_zeros = np.zeros((int(diff/2)-diff,vol00.shape[1],vol00.shape[2]))
            vol00 = np.concatenate((a_zeros,vol00,p_zeros),axis = 0)


           # vol00 = vol00[int(-diff/2):vol00.shape[0]+diff-int(diff/2),:,:]
    # pdb.set_trace()
    ...
    if pad_shape:
        vol50, _ = pad(vol50, pad_shape)
        vol00, _ = pad(vol00, pad_shape)

    if add_feat_axis:
        vol50 = vol50[..., np.newaxis]
        vol00 = vol00[..., np.newaxis]

    if resize_factor != 1:
        vol50 = resize(vol50, resize_factor)
        vol00 = resize(vol00, resize_factor)
...
#    vol50 = (vol50-vol50.min())/(vol50.max()-vol50.min())
#    vol00 = (vol00-vol00.min())/(vol00.max()-vol00.min())
    return np.concatenate((vol50,vol00), axis=0)
