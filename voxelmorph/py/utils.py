...
#    """
#    
#    data_dir = './data_pre/npdata/'
#    pid = filename.split(':')[0]
#    filename = filename.split(':')[1]
#    filename = filename.split(' ')
#    vol50 = np.load(data_dir+filename[0]+'.npy')
#    vol00 = np.load(data_dir+filename[1]+'.npy')
#    #pdb.set_trace()
#    vol50max = vol50.max()
#    vol00max = vol00.max()
#    print(vol50.max())
...
#        if name != 'BODY':
#        vol00 = vol00[np.newaxis, ...]
#    return np.concatenate((vol50,vol00), axis=0), vol50max,vol00max

def load_volfile_withbitmap3(
    filename,
    organname,
    np_var='vol',
    add_batch_axis=False,
    ...
    data_dir = './data_pre/npdata/'
    #pdb.set_trace()
    structure_dir = './data_pre/bitmaps_npdata/'
    pid = filename.split(':')[0]
    filename = filename.split(':')[1]
    filename = filename.split(' ')
    ...
    str50 = []
    str00 = []
    for name in organname:
        curr_dir = structure_dir +name+'/'
#            print(filename)
        tmp1 = np.load(curr_dir+filename[0]+'.npy')
        tmp2 = np.load(curr_dir+filename[1]+'.npy')
#            print(tmp1.shape)
#            print(tmp2.shape)
        if np.all(tmp1 ==1) or np.all(tmp2 ==1):
            str50.append(np.ones(tmp1.shape))
            str00.append(np.ones(tmp2.shape))
        else:
            str50.append(tmp1)
            str00.append(tmp2)
...
#    v50bitmap = np.load(bitmap_dir+filename[0]+'.npy')
#    v00bitmap = np.load(bitmap_dir+filename[1]+'.npy')
#    vol50[np.where(v50bitmap ==0)] = -1000
#    vol00[np.where(v00bitmap==0)] = -1000
    
    vol50 = (vol50+1000)/3000.
    vol00 = (vol00+1000)/3000.
    if pad_shape:
        vol50, _ = pad(vol50, pad_shape)
        vol00, _ = pad(vol00, pad_shape)
    ...
    if add_batch_axis:
        ...
        vol00 = vol00[np.newaxis, ...]
#    pdb.set_trace()
    str50 = np.array(str50)
    str00 = np.array(str00)
#    print(str50.dtype)
#    print(str00.dtype)
#    str50 = str50[...,np.newaxis]
#    str00 = str50[...,np.newaxis]
#    if len(str50)==0  or len(str00)==0:
#        pdb.set_trace()
#        str50 = np.zeros(vol50.shape[:-1])
#        str00 = np.zeros(vol50.shape[:-1])
#    else:
#        str50 = np.array(str50)
#        str00 = np.array(str00)


    return [np.concatenate((vol50,vol00), axis=0),str50,str00]

def load_volfile_withbitmap2(
    filename,
    organname,
    np_var='vol',
    add_batch_axis=False,
    ...
    data_dir = './data_pre/npdata/'
    #pdb.set_trace()
    bitmap_dir = './data_pre/bitmaps_npdata/'+organname[0]+'/'
    structure_dir = './data_pre/bitmaps_npdata/'
    pid = filename.split(':')[0]
    filename = filename.split(':')[1]
    filename = filename.split(' ')
...
def load_volfile_withbitmap(
    ...
    data_dir = './data_pre/npdata/'
    bitmap_dir = './data_pre/bitmaps_npdata/'+organname+'/'
    pid = filename.split(':')[0]
    filename = filename.split(':')[1]
    filename = filename.split(' ')
    vol50 = np.load(data_dir+filename[0]+'.npy')
