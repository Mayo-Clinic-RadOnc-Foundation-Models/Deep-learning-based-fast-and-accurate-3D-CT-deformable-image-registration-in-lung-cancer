...
#        pdb.set_trace()
        alldata = [py.utils.load_volfile_withbitmap3(vol_names[i],organ_names, **load_params) for i in indices]
#        pdb.set_trace()
        alldata = sum(alldata,[])
        imgs = np.array(alldata[::3])
        str50 = np.array(alldata[1:len(alldata):3])
        str00 = np.array(alldata[2:len(alldata):3])
#        if str50.shape[1] != 5:
#            pdb.set_trace()
        #imgs = np.array(imgs)
        vols = torch.Tensor(imgs)
        
        yield vols,torch.Tensor(str50),torch.Tensor(str00)

def volgen_withbitmap2(
    vol_names,
    organ_names,
    batch_size=1,
    segs=None,
    ...
    while True:
        ...
        load_params = dict(np_var=np_var, add_batch_axis=True, add_feat_axis=add_feat_axis,
                           pad_shape=pad_shape, resize_factor=resize_factor)
#        pdb.set_trace()
        alldata = [py.utils.load_volfile_withbitmap2(vol_names[i],organ_names, **load_params) for i in indices]
#        pdb.set_trace()
        alldata = sum(alldata,[])
        imgs = np.array(alldata[::3])
        str50 = np.array(alldata[1:len(alldata):3])
...
def volgen_withbitmap(
    ...
    while True:
        ...
        imgs = [py.utils.load_volfile_withbitmap(vol_names[i],organ_names, **load_params) for i in indices]
        imgs = np.array(imgs)
        vols = torch.Tensor(imgs)
...
def volgen(
    ...
    while True:
        ...
        #indices = np.random.randint(len(vol_names),size = 1)
        indices = np.random.randint(len(vol_names), size=batch_size)
...
def scan_to_scan_ctlung(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    Generator for scan-to-scan_ctlung registration.
    ...
    while True:
       ...
       # if not no_warp and zeros is None:
       #     shape = scan1.shape[1:-1]
       #     zeros = torch.Tensor(np.zeros((batch_size, *shape, len(shape))))
        invols = torch.stack((scan1,scan2),axis = 1) #size batchsize*2*z*y*x*1
        outvols = torch.stack((scan2, scan1),axis = 1) if bidir else scan2[:,None]
        #if not no_warp:
        #    outvols.append(zeros)

        yield (invols, outvols)

def scan_to_scan_ctlung_withbitmap3(vol_names,organ_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    zeros = None
#    pdb.set_trace()
    gen = volgen_withbitmap3(vol_names,organ_names, batch_size=batch_size, **kwargs)
#    pdb.set_trace()
    while True:
        scanall,scan1_str,scan2_str = next(gen)
        scan1 = scanall[:,0,:,:,:]
        scan2 = scanall[:,1,:,:,:]
        ...
        invols = torch.stack((scan1,scan2),axis = 1) #size batchsize*2*z*y*x*1
        outvols = torch.stack((scan2, scan1),axis = 1) if bidir else scan2[:,None]
        invols_str = torch.stack((scan1_str,scan2_str),axis = 1)
#        pdb.set_trace()
        outvols_str = torch.stack((scan2_str, scan1_str),axis = 1) if bidir else scan2_str[:,None]
        yield (invols, outvols,invols_str,outvols_str)


def scan_to_scan_ctlung_withbitmap2(vol_names,organ_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    zeros = None
#    pdb.set_trace()
    gen = volgen_withbitmap2(vol_names,organ_names, batch_size=batch_size, **kwargs)
#    pdb.set_trace()
    while True:
        scanall,scan1_str,scan2_str = next(gen)
        scan1 = scanall[:,0,:,:,:]
...
def scan_to_scan_ctlung_withbitmap(vol_names,organ_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    zeros = None
    #pdb.set_trace()
    gen = volgen_withbitmap(vol_names,organ_names, batch_size=batch_size, **kwargs)
    #pdb.set_trace()
    while True:
        scanall = next(gen)
        scan1 = scanall[:,0,:,:,:]
